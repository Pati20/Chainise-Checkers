...
public class ThreadOfServer extends Thread {
    ...
    public ThreadOfServer(int pport, int clientNumber, ServerSide sserverSide, ServerSocket sserverSocket) {
        this.port = pport;
        try {
    ...
    private String DoSomething(String line) {
        ...
        switch (comand.get(0)) {
            ...
            case "GETGAMES"://GETGAMES + CLIENT_ID + BOARD_ID
                for (OldBoard b : serverSide.boards) {
            ...
            case "CONECTTOGAME"://CONECT + CLIENT_ID + BOARD_ID(od 0)
                ...
                if (serverSide.boards.get(boardID).numberOfHumans == serverSide.boards.get(boardID).currentHumanCount) {
                    for (ThreadOfServer t : serverSide.threadServer) {
    ...
    public int addNewBoard(int playerID, int numberOfHuans, int numberOfBots) {
        ...
        serverSide.boards.add(new OldBoard(witch, playerID, numberOfHuans, numberOfBots));
        return witch;
    }
